...
HANDLE Base::CreateFIFO(CString PipeName)
{
	CString buffer;
...
HANDLE Base::WaitFIFO(CString PipeName)
{
	CString buffer;
	buffer.Format(L"\\\\.\\Pipe\\%s", PipeName.GetBuffer());
...
BOOL Base::ConnectFIFO(HANDLE h_Pipe)
{
...
BOOL Base::CloseFIFO(HANDLE h_Pipe)
{
...
private:
	...
	BOOL CreateSharedMemory(unsigned long DataSize, CString ServerWriteOverName, CString ClientReadOverName)
	{
		...
		{
			hShipFileMapping = CreateFileMapping(INVALID_HANDLE_VALUE, NULL, PAGE_READWRITE, 0, DataSize, SharedMemoryName);
		}
