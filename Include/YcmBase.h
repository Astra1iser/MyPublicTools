...
*/
	...
	如何使用命名管道:
	1.HANDLE h_Pipe = CreateFIFO(L"mypipe");  //首先发送者创建命名管道
	2.WaitFIFO(L"mypipe"); //接收者等待管道信号,这个函数如果执行时没有检测到对应的命名管道,则会直接返回,如果检测到了对应的命名管道,
	则等待发送者使用ConnectFIFO()建立链接并创建管道文件
	3.等待1和2完成时,则接收者和发送者之间可使用ReadFIFO()和WriteFIFO进行通信
